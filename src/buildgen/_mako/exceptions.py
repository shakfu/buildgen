...
from buildgen._mako import compat
from buildgen._mako import util
...
class RichTraceback:
    ...
    def _init(self, trcback):
        ...
        from buildgen._mako import template as mako_template
        ...
        for filename, lineno, function, line in rawrecords:
            ...
            try:
                ...
                try:
                    info = mako_template._get_module_info(filename)
        ...
        if not self.source:
            for idx in range(len(new_trcback) - 1, 0, -1):
                if new_trcback[idx][5]:
                    self.source = new_trcback[idx][7]
                    self.lineno = new_trcback[idx][5]
                    break
            else:
                if new_trcback:
                    try:
...
def text_error_template(lookup=None):
    ...
    return mako_template.Template(
        r"""
<%page args="error=None, traceback=None"/>
<%!
    from buildgen._mako.exceptions import RichTraceback
%>\
<%
...
def _install_pygments():
    ...
    from buildgen._mako.ext.pygmentplugin import syntax_highlight  # type: ignore[import-untyped]  # noqa
    from buildgen._mako.ext.pygmentplugin import pygments_html_formatter  # type: ignore[import-untyped]  # noqa


def _install_fallback():
    global syntax_highlight, pygments_html_formatter
    from buildgen._mako.filters import html_escape
...
def html_error_template():
    ...
    return mako_template.Template(
        r"""
<%!
    from buildgen._mako.exceptions import RichTraceback, syntax_highlight,\
